...
Buffalo Groupe provides four core logo formats, each available in Midnight Sky (default) or white:

1. **Mark only** – Buffalo silhouette alone, no type
   - Use for: Icon, favicon, small social avatars, mark applications
   - Minimum size: 32px (web)

2. **Horizontal, no stars, no tagline** – "buffalo groupe" stacked right of mark
   - Use for: Headers, footers, primary brand application
   - Best for: Wider spaces (desktop, landscape layouts)

3. **Vertical stack with stars, no tagline** – Stars above, buffalo and "buffalo groupe" stacked
   - Use for: Narrower spaces, mobile, vertical layouts
   - Spacing: Stars centered above buffalo mark

4. **Horizontal with stars and tagline** – "we create escape" arched above, stars, buffalo mark, "buffalo groupe" horizontal
   - Use for: Full brand identity, hero sections, marketing materials
   - Best for: Wider spaces; shows complete brand promise

### Color Versions

- **Midnight Sky** (default on light backgrounds)
- **White** (on dark backgrounds, photos, colored surfaces)

### File Reference

| Usage | File |
|-------|------|
| Mark only | `BGLogo_MarkOnly_rgb.png` |
| Horizontal, no stars | `BGLogo_NoStars_NoTag_HorizStack_rgb.png` |
| Vertical with stars | `BGLogo_WithStarsNoTag_VertStack_rgb.png` (or `_Wht_` for white) |
| Horizontal with stars and tagline | `BGLogo_wStarsAndTag_Long_rgb.png` (or `_Wht_` for white) |

### Logo Rules

- Maintain spacing equal to the height of the buffalo mark around all versions
- Exception: When mark is bottom-aligned with page or bounding edge, standard clearance not required
- Never stretch, distort, or alter proportions
- Never rotate the logo (fixed orientation only)
- Use only approved color variants (Midnight Sky or white)
- Mark may appear alone in icon contexts; full lockups require "buffalo groupe" type
